package handler

import (
	"github.com/SyamSolution/grule-service/config"
	"github.com/SyamSolution/grule-service/internal/model"
	"github.com/SyamSolution/grule-service/internal/usecase"
	"github.com/SyamSolution/grule-service/internal/util"
	"github.com/gofiber/fiber/v2"
	"go.uber.org/zap"
)

type discount struct {
	discountUsecase usecase.DiscountExecutor
	logger          config.Logger
}

type DiscountHandler interface {
	CheckDiscount(c *fiber.Ctx) error
}

func NewDiscountHandler(discountUsecase usecase.DiscountExecutor, logger config.Logger) DiscountHandler {
	return &discount{
		discountUsecase: discountUsecase,
		logger:          logger,
	}
}

func (h *discount) CheckDiscount(c *fiber.Ctx) error {
	var discountRequest model.Discount
	if err := c.BodyParser(&discountRequest); err != nil {
		h.logger.Error("failed to parse request", zap.Error(err))
		return c.Status(fiber.StatusBadRequest).JSON(model.ResponseWithoutData{
			Meta: model.Meta{
				Code:    400,
				Message: util.ERROR_NOT_FOUND_MSG,
			},
		})
	}

	discountAmount, err := h.discountUsecase.CheckDiscount(&discountRequest)
	if err != nil {
		h.logger.Error("failed to check discount", zap.Error(err))
		return c.Status(fiber.StatusInternalServerError).JSON(model.ResponseWithoutData{
			Meta: model.Meta{
				Code:    500,
				Message: util.ERROR_BASE_MSG,
			},
		})
	}

	return c.Status(fiber.StatusOK).JSON(model.Response{
		Data: discountAmount,
		Meta: model.Meta{
			Code:    200,
			Message: util.SUCCESS_RESPONSE_MSG,
		},
	})
}
